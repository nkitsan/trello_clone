...
###Сущности
Основные сушности приложения описаны в таблице выше. Класс пользователя (User) описан для базовой версии, в расширенной понадобятся поля еще и для хранения данных авторизации, как почтовый адрес и пароль. У пользователя есть поля для хранения основных данных, как еженедельный список задач (weekly_task) - дефолтный список задач у пользователя, трекер привычек (habbit_tracker), календарь (calendar) и массив различных списков(task_list). Класс List представляет список различных задач. В свою очередь для задач существуют классы как PrivateTask (приватные задачи) и PublicTask (публичные задачи), основное отличие в них это возможность указывать исполнителя и отсутствие напоминания в публичных задачах. Даты дедлайнов храняться в поле deadline классов приватных и публичных задач (для публичных задач будет автоматическое уведомление в начале дня дедлайна и за час до него). Класс Task будет использоваться для подзадач, поэтому имеет только поле имени и статуса. Класс HabbitTracker нужен нам для хранения списка привычек пользователя, каждая привычка которого представлена в классе Habbit. Класс Calendar (для представления важных дат календаря) cодержит массив объектов класса Date, который используется для хранения создаваемых пользователем событий в календаре.
