...
* Задачи календарного типа (с повторяемоcтью раз в месяц или реже). Примеры: поставить напоминание об оплате за интернет (в течение каждого часа, пока оно не будет выполнено), записать время приема у врача и напомнить за день и за час до приема, занести дату дня рождения двоюродной сестры и напомнить в этот день. 
* Ежедневные задачи (такие, которые можно назвать привычками). Примеры: привычка изучение иностранного языка с напоминанием в конце дня, прием таблеток с напоминанием 3 раза в день, привычка не есть сладкое (напоминать вечером отметить задание выполненным).
